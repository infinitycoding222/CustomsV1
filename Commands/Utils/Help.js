        ...
        if (command) {
            ...
            let e2 = new MessageEmbed()
                ...
                .addField('Other', [
                    ...
                    `Client Permissions: \`${cmd.clientPerms || "None"}}\``,
                    `User Permissions: \`${cmd.userPerms || "None"}\``
                ], true)
                .setColor(this.client.config.color)
                .setFooter(this.client.config["config"].copyright)
            message.channel.send(e2)
            console.log(JSON.stringify(cmd.clientPerms), JSON.stringify(cmd.userPerms))
        } else {
            let categories;
            ...
            for (const category of categories) {
                e.addField(`< ${this.client.utils.toProperCase(category)} > - [ ${this.client.commands.filter(cmd => cmd.category === category).size} ]`, `${this.client.commands.filter(cmd => cmd.category === category).map(cmd => `\`${cmd.name}\``).join(" ")}`)
            }
